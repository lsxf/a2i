...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Release-v1.12.21-2563EB" alt="Release v1.12.21">
...
| **Shizuku** | 微信回复 / PIN 自动解锁需要；锁屏仍隐藏验证码时可选 | 以 ADB shell 权限操作微信、锁屏，并在短信通知被系统脱敏时读取对应短信原文 | [Shizuku 官网](https://shizuku.rikka.app/download/) / [GitHub Releases](https://github.com/RikkaApps/Shizuku/releases) |
...
| Shizuku | 使用微信回复、PIN 解锁或锁屏验证码恢复时 | 以 ADB shell 权限执行输入、剪贴板、锁屏，以及按通知时间读取对应短信原文 |
...
| 锁屏后收到的验证码变成 `******` | 优先把系统和短信 App 的锁屏通知改为“显示内容”；如果希望继续隐藏锁屏内容，再启动并授权 Shizuku。无法确认真实原文时 GotMsg 会安全过滤 |
...
- 短信通知中的验证码被锁屏策略替换为星号时，GotMsg 仅在 Shizuku 已授权后按通知时间读取收件箱附近的短信，并只接受可确认的验证码原文；恢复结果与普通通知一样进入现有转发、日志和失败重发链路，不会另建短信数据库副本。无法确认时直接过滤。
