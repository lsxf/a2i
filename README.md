...
<p align="center">
  <img src="screenshots/home-latest.png" alt="GotMsg 1.16.7 首页" width="360">
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Android-14%2B-3DDC84?logo=android&logoColor=white" alt="Android 14+">
  <img src="https://img.shields.io/badge/Release-v1.16.7-2563EB" alt="Release v1.16.7">
...
各通道都支持添加多条配置、单独启停和排序；Bark、ntfy、Meow 和电邮配置还可单独发送测试。“首个成功即止”适合相同通道的备用目标；关闭后会向该通道所有已启用配置广播。不同通道之间彼此独立并行发送，只要任一已配置通道成功，整条通知就记为发送成功；没有配置 Bark 不会产生 Bark 失败记录。
...
- 公告会在原手机显示本地系统通知；打开 GotMsg 的全局转发开关后，还会直接转发到已启用的 **Bark、ntfy、Meow 和邮件**渠道。不经过通知监听器回收，因此不会形成自身通知循环，也不会生成回复链接。
...
| 锁屏后收到的验证码变成 `******` | 优先把系统和短信 App 的锁屏通知改为“显示内容”；如果希望继续隐藏锁屏内容，再启动并授权 Shizuku。Shizuku 未安装、未运行或未授权时，GotMsg 不会转发星号正文，而会向已启用的 Bark、ntfy、Meow 和邮件通道发送“验证码无法读取”提醒，请回到安装 GotMsg 的原手机查看 |
...
- 短信通知中的验证码被锁屏策略替换为星号时，GotMsg 会先尝试短信 App 提供的受控验证码字段；仍无法恢复时，仅在 Shizuku 已授权后按通知时间读取收件箱附近的短信，并只接受可确认的验证码原文。Shizuku 未安装、未运行或未授权时，GotMsg 会向已启用的 Bark、ntfy、Meow 和邮件通道发送一条不含验证码及星号正文的安全提醒；Shizuku 已就绪但仍无法确认原文时直接过滤。不会另建短信数据库副本。
